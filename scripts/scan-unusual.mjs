...
import { TICKERS, loadAiUsageState, writeAiUsageState } from "./build.mjs";
...
// Current-metrics explanation snapshot. This used to call Gemini and cache by
// contract identity, which could quote old volume/tape/premium after a re-flag.
// The explanation is entirely mechanical, so deterministic prose is both
// fresher and free. The sidecar remains for operator inspection/compatibility.
...
// ---------------------------------------------------------------------------
// Current unusual-flow explanations
// ---------------------------------------------------------------------------

function flowExplanationKey(contract) {
  return `${contract.symbol}|${contract.side}|${contract.strike}|${contract.expSec}`;
}

// Rebuild every visible note from the metrics carried by this scan. New hits
// therefore quote current volume/tape/premium; carried earlier-session hits
// remain tied to their own scannedAt metrics. No model, cache hit, or external
// causal claim participates in this layer.
function attachFlowExplanations(mergedTickers, scannedAt) {
  const entries = {};
  for (const ticker of mergedTickers || []) {
    for (const contract of ticker?.contracts || []) {
      const note = buildFlowExplanation(contract, ticker.spot);
      contract.note = note;
      entries[flowExplanationKey(contract)] = {
        note,
        generatedAt: scannedAt,
        sourceScannedAt: contract.scannedAt || scannedAt,
        expSec: contract.expSec,
      };
    }
  }
  return { updatedAt: scannedAt, mode: "deterministic-v1", entries };
}

async function writeFlowExplanations(snapshot) {
...
async function runVolumePass({
  ...
  // volume-flags.json / volume-history.json — so an out-of-band caller can read
  // the live volume board without disturbing the hourly snapshot cadence the
  // hour-over-hour bucket deltas depend on. The hourly full scan (the only
  // caller today) persists.
  persist = true,
}) {
  ...
  // Skip all persistence when the caller only wants the in-memory rows — see
  // the `persist` param above.
  if (persist) {
...
async function main() {
  ...
  // AI usage totals are shared with the daily build and the post-close heatmap
  // recap. Flow notes no longer spend AI, but carry/rewrite the shared state so
  // this owner never drops another producer's daily accounting.
  ...
  // Explain each contract from the current metrics before serializing. The
  // note cannot lag a re-flag or invent a catalyst.
  const flowCache = attachFlowExplanations(mergedTickers, scannedAt);
...
// The intraday volume pass + the shared data dir. Importing this module must
// NOT trigger a live scan — only run main() when invoked directly (mirrors
// build.mjs's entry guard).
export {
  runVolumePass, DATA_DIR,
};
