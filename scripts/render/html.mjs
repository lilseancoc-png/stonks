  ...
  return `<section class="card" id="picks-section">
    ...
    <details class="picks-howto">
      ...
      <div class="picks-howto-body">
        ...
        <p><b>How the market tape moves the picks.</b> Macro enters two ways: as direct signals on every name (above), and as a market <b>regime</b> that changes the engine&rsquo;s whole posture.</p>
...
function overnightSection() {
  ...
  return `<section class="card" id="overnight-section">
    ...
    <p class="hint">Correlation (r) and sensitivity (&beta;) are computed from up to 150 trading days of daily-return overlap; &beta; &times; the peer&rsquo;s move is a rough implied read, not a forecast. Foreign closes can lag the US session by up to a day. Not financial advice.</p>
  </section>`;
}
...
function volumeFlagsSection() {
  ...
  return `<section class="card vol-card" id="vol-section">
    ...
    <p class="hint">Each card also carries a <strong>follow-the-case verdict</strong> — whether the volume evidence says to <em>follow</em> the bull or bear case (heavy volume confirmed the move), <em>wait for confirmation</em> (heavy participation but no decisive direction yet), or <em>not follow</em> it (a weak move or a likely fakeout, prone to fading). Expand a ticker to read the verdict's reasoning in full.</p>
    <p class="hint">The build's current <strong>★ Top Picks</strong> are pinned in their own group at the top so you can track flow on just those names; the rest are grouped by sector and <strong>collapsed by default</strong> — click a sector header to open it, then a ticker to expand its hour-by-hour breakdown with the reasoning. Each row's one-line summary shows its strongest flag, bullish/bearish lean, peak hour ratio, and EOD move. <em>Group by sector</em> and <em>Expand all</em> toggle the layout.</p>
    <div class="vol-live" id="vol-live-section">
      <div class="vol-live-head">
        <label class="vol-live-toggle">
          <input type="checkbox" id="vol-live-toggle" />
          <span>Live tracking</span>
        </label>
        <span class="vol-live-hint">Polls live quotes every 30s while this tab is open — every tracked name's cumulative day volume vs. the volume <em>expected by this point of the session</em> (same U-shaped curve as the hourly scan), ranked by pace. ≥1.2× runs hot; ≥1.5× is exceptional. Each row carries the latest scan's dealer-gamma read (net GEX, flip, call/put walls + the OI tracker's squeeze score when ≥3) with flip/wall distances recomputed against the live spot. The <strong>Now</strong> column is the same read over just the trailing ~10 minutes — shares traded in that window vs. the volume <em>usual</em> for that exact slice of the session (hover/tap for the amounts). The <strong>Play</strong> chip is an execute-or-wait read graded <em>at this moment</em>: it goes <em>Bullish</em>/<em>Bearish</em> only when the trailing-window move (day move until the window warms up), the scanner's S/R break, and the dealer-gamma backdrop line up <em>and</em> the right-now volume confirms (≥1.2×); anything less is a Wait (tap the chip for the full reasoning).</span>
        <span class="vol-live-state" id="vol-live-state" aria-live="polite"></span>
      </div>
      <div id="vol-live-board" class="vol-live-board" hidden></div>
    </div>
