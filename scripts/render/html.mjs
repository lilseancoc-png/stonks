...
function hotStocksSection() {
  // The LIVE, volume-driven Day Trades board. A roster of concrete swing/scalp
  // ideas — each with a FIXED entry / take-profit / stop-loss — generated by the
  // hourly scan off the heaviest-volume names and then tracked live: the browser
  // polls /api/quotes every 30s for each open trade's P/L and flags a TP/SL
  // touch the instant it happens; the scan makes the close durable and files the
  // win/loss into the history. Below, the live volume leaders the ideas come
  // from. Contrast Top Picks: that is the longer-dated narrative/fundamentals
  // play; this is the tape right now.
  return `<section class="card hot-card" id="hot-section">
    <header class="card-header">
      <h2 class="card-title">Day trades <span class="dte-badge" title="Live volume-driven swing/scalp trades, tracked to their take-profit / stop-loss">LIVE</span></h2>
      <span class="vol-live-state" id="hot-live-state" aria-live="polite"></span>
    </header>
    ${infoNote('How the live Day Trades board works', `<p>A live roster of concrete <strong>swing</strong> and <strong>scalp</strong> ideas, mined every hour from the names <strong>trading the heaviest volume</strong> with a clean directional read (a confirmed 20-day support/resistance break, the day's move, dealer gamma). Each trade ships a <strong>fixed entry, take-profit and stop-loss</strong> with its reward:risk — a <strong>swing</strong> rides a 20D breakout for up to three sessions, a <strong>scalp</strong> is a tight intraday momentum trade that closes by the bell.</p>
    <p>The board is <strong>live</strong>: while this tab is open it refreshes every 30s, marking each open trade to the tape (its P/L in % and in <strong>R</strong> multiples) and flagging the moment price touches the take-profit or stop. When a level is hit the trade <strong>disappears from the active list and is filed into the history</strong> as a win or loss — the <em>History &amp; results</em> view keeps the running win rate, average P/L and average R. Trades are on the <strong>underlying</strong> (a stock move, not an options model). Not financial advice. For the longer-dated, narrative-driven play, see <strong>Top Picks</strong>.</p>`)}
    <div class="hot-dt-tabs" id="hot-dt-tabs" role="tablist" aria-label="Day trades view">
      <button type="button" class="vol-pill is-on" data-hot-view="active" role="tab" aria-selected="true">Active trades</button>
      <button type="button" class="vol-pill" data-hot-view="history" role="tab" aria-selected="false">History &amp; results</button>
    </div>
    <div id="hot-active-view">
      <div id="hot-dt-summary" class="hot-dt-summary" aria-live="polite" hidden></div>
      <div id="hot-active" class="hot-active" aria-live="polite">Loading live day trades…</div>
    </div>
    <div id="hot-history-view" hidden>
      <div id="hot-history-stats" class="hot-dt-summary" aria-live="polite" hidden></div>
      <div id="hot-history" class="hot-history-list" aria-live="polite"></div>
    </div>
    <details class="hot-volboard">
      <summary>Live volume leaders — what's trading heaviest right now (where the ideas come from)</summary>
