...
function volumeFlagsSection() {
  ...
  return `<section class="card vol-card" id="vol-section">
    ...
    <p class="hint">Tickers are grouped by sector and collapsed to a one-line summary — its strongest flag, bullish/bearish lean, peak hour ratio, and EOD move. Click a ticker to expand the hour-by-hour breakdown with the reasoning, or a sector header to collapse the group. <em>Group by sector</em> and <em>Expand all</em> toggle the layout.</p>
