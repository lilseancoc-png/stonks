  ...
  return `<section class="card" id="picks-section">
    ...
    <details class="picks-howto">
      ...
      <div class="picks-howto-body">
        <p>A <b>fixed, auditable</b> grading system. Every tracked name receives a directional asset grade from four pillars (<b>Fundamentals</b>, <b>Technicals</b>, <b>Mechanicals</b>, <b>Narrative</b>) plus a separate IV-cost adjustment. <b>Entry timing is an execution overlay, not part of that grade</b>: a strong thesis can stay strong while the correct action is Wait or Avoid. Names must clear a fixed conviction floor, the thesis-quality review, and the execution gate to become actionable. The list is deliberately allowed to be <b>short, or empty</b>, on a poor day &mdash; the engine would rather hold cash than pad it. Each card has a <b>Recommendation&nbsp;&#8644;&nbsp;Grade</b> toggle &mdash; flip to Grade to audit every signal behind the score &mdash; plus a named entry strategy, a layered exit ladder, and a same-sector peer comparison. The <b>Track record</b> tab marks past picks to market (modeled option P&amp;L).</p>
        <p><b>The grade and execution read.</b> The asset grade is the sum of four pillars, each clamped to &plusmn;5, plus a direction-aware IV-cost term (&minus;2&hellip;+1). Its thresholds are fixed rather than recomputed from the day&rsquo;s universe. The separate timing score runs from &minus;8 to +2 and decides whether to Go, Wait for a named trigger, or Avoid.</p>
        ...
        <p><b>&bull; Regime-conditioned proof.</b> Countertrend calls in risk-off and countertrend puts in risk-on need a reclaim, full momentum confirmation and clean structure. The bar also rises for the crowded side: an already-extended call in strong risk-on, or put in risk-off, needs full confirmation and qualifying payoff. A clean pullback aligned with the tape is not penalized merely because the regime agrees.</p>
        <p><b>Tiers &amp; sizing.</b> Tiers use fixed absolute bars: |grade| &ge;7 is Strong, 4&ndash;6 is Call/Put, and below 4 is No&nbsp;Trade. Clearing the grade bar does not guarantee an actionable pick &mdash; thesis quality and the execution gate still bind, so the list can remain short or empty. Size is risk-based, not flat: each pick is weighted inverse to the premium it would lose to its stop, tilted by conviction, and the book&rsquo;s overall gross is trimmed when the recent <i>realized</i> track record is negative.</p>
...
function indexCalSection() {
  ...
  return `<section class="card" id="index-cal-section">
    ...
    ${infoNote('How to read the index calendar', `<p>Use this as a <b>cross-asset participation record</b>, not just a red/green diary. The six risk sleeves &mdash; <b>SPY, QQQ, IWM, SMH, DIA and VXUS</b> &mdash; show whether equity strength is broad or concentrated; <b>TLT</b> and <b>GLD</b> add rate and defensive context; <b>VIX</b> confirms calm or stress. Start with the month tape read, then click a trading day to compare all nine instruments. Green means an up day for every instrument except VIX, where green means volatility eased and red means stress rose. The displayed percentage always remains the true close-to-close move. Today updates through the session and finalizes at the 4&nbsp;pm&nbsp;ET close. Not financial advice.</p>`)}
    ...
    <div class="quant-playbook">
      ...
      <div class="quant-tbl-wrap"><table class="quant-tbl quant-play-tbl">
        ...
        <tbody>
          ...
          <tr><td>Regime shifts (calm&rarr;crisis, range&rarr;trend)</td><td>Re-tune every threshold before trusting a flag</td><td>A 2&sigma; fade that works in a calm range gets run over in a trend; a &ldquo;rich&rdquo; VRP is normal when vol is high &mdash; the strip at the top shows the bars in force</td></tr>
