...
function unusualFlowSection() {
  ...
  return `<section class="card flow-card" id="flow-section">
    ...
    <div id="flow-body" class="flow-body">
      ${infoNote('What counts as unusual flow?', `<p>Block/sweep flow: 5–50% OTM contracts that picked up at least 2,000 contracts of volume this hour (4,000 if expiring within 2 weeks) with vol &gt; OI. The kind of single-shot directional activity that can reveal informed positioning. The <strong>decision queue</strong> only treats prints at or above the midpoint as aggressive premium, separates one-sided demand from two-sided event risk, and refuses to call an old or closed-session scan executable. Each chip shows the <strong>volume-to-OI multiple</strong> (e.g. 4×, the canonical unusual read) and the <strong>premium that hit this hour</strong>. A 🔥 ×N badge means the same contract has flagged that many times in the last 5 trading days — recurring interest. Chips tint with the hourly delta, and <strong>≥25k/hr prints go violet</strong> — whale-sized flow that shouldn't blend in. Flow is a watchlist input, not an entry: calls can be sold, puts can be hedges, and every read still needs confirmation from the underlying price, volume, and structure. Hourly scan, front 2 expirations.</p>`)}
...
function oiTrackerSection() {
  ...
  return `<section class="card oi-card" id="oi-section">
    ...
    <div id="oi-body" class="oi-body">
      ${infoNote('How the OI ladder & squeeze score work', `<p>Start with the <strong>positioning desk</strong>: it identifies the strongest current squeeze candidate, distinguishes a 4–5/5 setup from a watch-only concentration, and states the price/flow evidence that must confirm or invalidate it. The ladder below shows the top 12 highest-open-interest strikes (calls + puts) across this week's and next week's expirations, with each side sorted closest-to-spot first. The <strong>Gamma Squeeze Score</strong> (0–5) awards heavy near-the-money call OI · C/P ratio ≥ 2:1 · call wall Vol/OI ≥ 1.5× · spot within 10% of the call wall · aggressive ask-side call flow today. A score of <strong>4–5</strong> is a potential setup, not an entry. Strikes with <strong>OI &gt; 1000</strong> get a chip; ΔOI day-over-day chips fire at <strong>+30%</strong> (new positioning) and <strong>+100%</strong> (very aggressive). Open interest is a lagged positioning map: price must still confirm at the wall, and a pre-market or prior-session snapshot must not be mistaken for live order flow. Twice-daily scan: pre-market (~08:30 ET) and EOD (~19:00 ET).</p>`)}
