...
function overnightSection() {
  ...
  return `<section class="card" id="overnight-section">
    ...
    <p class="hint">Asian cash markets and FX trade and close <em>before</em> the US opens, so an overnight move in a foreign peer is a leading read on its US counterpart — Samsung &amp; SK Hynix selling off in Seoul flags memory names like MU; a yen-carry unwind in Tokyo flags broad US risk. Each tile shows a foreign symbol&rsquo;s last completed session move and the US tickers it leads (click a tag to grade it). Captured at build time &mdash; the 9:30&nbsp;ET build is the first to see the just-closed Asian session.</p>
