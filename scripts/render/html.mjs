...
function marketAnalysisSection() {
  ...
  return `<section class="card" id="market-section">
    ...
    ${infoNote('How to read the scenario overlay', `<p>The scenario layer is an <b>unvalidated deterministic score</b>, not a calibrated forecast. The 0&ndash;100 weights rank constructed paths relative to each other; they are not probabilities. Sample size is the calendar of upcoming events and the 16-axis tape already on this page &mdash; there is no out-of-sample hit rate behind the weights. Use it as a filter, then confirm with Grade, price and volume.</p>`)}
...
function trackRecordSection() {
  ...
  return `<section class="card" id="accuracy-section">
    ...
    <p class="hint">Modeled Black-Scholes marks at constant entry IV from the enrollment snapshot — not live chain fills. Start with the sample size and results below; win rates stay blank until enough resolved trades exist, and the page never invents them. Use the other views to inspect the ledger, cohorts and simulations.</p>
    ...
    <p class="picks-foot">Track record is informational, not a performance claim: option P&amp;L is a constant-IV Black-Scholes mark from the enrollment snapshot, not a live chain fill. The <b>$ profitability, equity curve, Simulator, and Monte&nbsp;Carlo</b> views are <b>modeled and hypothetical</b>. Not financial advice.</p>
  </section>`;
}
...
function buildTimelineSection() {
  return `<section class="card refresh-schedule" id="refresh-schedule-section">
    ...
    <div class="refresh-schedule-grid">
      ...
      <article class="refresh-schedule-card"><span>11:00 &amp; 15:30 ET</span><h3>Swing decision desks</h3><p>Runs Top Picks, Stock Picks, Sector Rotation and Leveraged ETFs. Chart vision runs at 11:00 only. Other full builds carry the last coherent decisions and ledgers unchanged.</p><em>Twice per market day &middot; aligned evidence</em></article>
...
function unusualFlowSection() {
  ...
  return `<section class="card flow-card" id="flow-section">
    ...
    <div id="flow-body" class="flow-body">
      ${infoNote('What counts as unusual flow?', `<p>Discovery stays broad: 5–50% OTM contracts that picked up at least 2,000 contracts of volume this hour (4,000 if expiring within 2 weeks) with vol &gt; OI remain visible as block/sweep candidates. The <strong>decision queue</strong> is stricter: it credits only ask/above-ask execution, requires material premium (normally $100k, or $50k when repeated, or $25k when aggressive demand spans multiple strikes), rejects quoted spreads wider than 35% of midpoint, and demands extra support for far-OTM or penny contracts. Midpoint prints remain evidence but are not called directional, and an old or closed-session scan is never called executable. Each chip shows <strong>volume-to-OI</strong> and hourly premium; 🔥 ×N marks repeats over five trading days. A single hour&rsquo;s flags are a <strong>small sample</strong> &mdash; treat one print as a watch, not a confirmed positioning shift. FINRA ATS volume is shown as a <strong>secondary, delayed context signal</strong> with dark-pool share of matching weekly consolidated volume and week-over-week change; it never changes the live flow direction or actionability call. Flow is a watchlist input, not an entry: calls can be sold, puts can be hedges, and every read still needs confirmation from the underlying price, volume, and structure. Hourly scan, front 2 expirations.</p>`)}
...
function gexSection() {
  ...
  return `<section class="card gex-card" id="gex-section">
    ...
    ${infoNote('What is gamma exposure (GEX)?', `<p>This is an open-interest-based <strong>GEX proxy</strong> per strike and expiration: <code>Γ × OI × 100 × spot² × 1%</code>, with Black-Scholes gamma computed from each contract's implied vol. The display applies the conventional call-positive / put-negative convention, so a cell is call GEX minus put GEX. That convention is useful for locating concentrations, walls, and a model-implied flip, but <strong>open interest does not reveal whether dealers are actually long or short each contract</strong>. The sign is therefore a scenario proxy, not observed dealer inventory. It is a snapshot of listed OI at one scan, not a time series and not a dealer-inventory count.</p>
