...
.skip-content { position: fixed; top: 8px; left: 12px; z-index: 1000; padding: 12px 18px; background: var(--surface); color: var(--text-strong); border: 2px solid var(--accent); border-radius: 8px; transform: translateY(-160%); }
.skip-content:focus { transform: none; }
#main-content { scroll-margin-top: 80px; }
.workspace-settings { position: relative; }
.workspace-settings-panel { display: flex; align-items: center; gap: 8px; }
.workspace-settings .auth-chip[hidden] { display: none; }
#workspace-settings-toggle { display: none; }
.workspace-pin-toggle { width: 100%; margin-top: 10px; min-height: 36px; border: 1px solid var(--border); border-radius: 6px; background: transparent; color: var(--muted-strong); font: 500 12px var(--font-sans); cursor: pointer; }
.workspace-pin-toggle[aria-pressed="true"] { color: var(--accent); border-color: var(--accent-line); }
.workspace-pin-toggle:disabled { opacity: .65; cursor: default; }
.workspace-pinned { padding: 12px 0 0; max-height: min(280px, 35vh); overflow-y: auto; }
.workspace-pinned[hidden] { display: none; }
.workspace-pinned h2 { margin: 0 0 8px; font: 600 11px var(--font-sans); color: var(--muted); }
.workspace-pin-row { display: flex; align-items: stretch; gap: 4px; }
.workspace-pin-link, .workspace-unpin { min-height: 36px; border: 0; border-radius: 6px; background: transparent; color: var(--text); cursor: pointer; }
.workspace-pin-link { flex: 1; min-width: 0; text-align: left; padding: 8px; font: 500 13px var(--font-sans); }
.workspace-pin-link[aria-current="page"] { background: var(--accent-tint-2); color: var(--accent); }
.workspace-unpin { flex: 0 0 36px; font-size: 20px; color: var(--muted); }
.workspace-pin-link:hover, .workspace-unpin:hover { background: var(--surface-2); }
.workspace-retry { display: block; margin: 12px auto 0; min-height: 44px; padding: 8px 20px; border: 1px solid var(--accent-line); border-radius: 8px; background: var(--accent-tint-1); color: var(--text-strong); cursor: pointer; font: 600 14px var(--font-sans); }
.workspace-retry:disabled { opacity: .65; cursor: wait; }
.calendar-context { margin: 12px 0; border: 1px solid var(--border); border-radius: 8px; padding: 0 12px; }
.calendar-context > summary { padding: 12px 0; min-height: 44px; cursor: pointer; font: 600 13px/1.5 var(--font-sans); color: var(--text-strong); }
#calendar-context-summary { margin-left: 10px; color: var(--muted-strong); font-weight: 400; }
.calendar-context .cal-briefing { margin-top: 0; }
@media (max-width: 1279px) {
  .site-header-lead { flex-shrink: 0; }
  .site-nav { flex-shrink: 0; }
  #workspace-settings-toggle { display: inline-flex; width: 44px; height: 44px; }
  .workspace-settings-panel { display: none; position: absolute; right: 0; top: calc(100% + 10px); width: 250px; max-width: calc(100vw - 24px); padding: 14px; border: 1px solid var(--border-strong); border-radius: 10px; background: var(--surface); box-shadow: 0 12px 30px rgb(0 0 0 / .25); flex-wrap: wrap; }
  .workspace-settings.is-open .workspace-settings-panel { display: flex; }
  .workspace-settings .time-zone-control { flex: 1; width: auto; padding: 0 8px; gap: 8px; }
  .workspace-settings .time-zone-control svg { display: block; }
  .workspace-settings .time-zone-control select { width: 100%; height: 44px; font-size: 16px; }
  .workspace-settings #theme-toggle { width: 44px; height: 44px; }
  .workspace-settings .auth-chip { width: 100%; justify-content: space-between; border-radius: 6px; }
  .workspace-settings .auth-chip .auth-name { display: inline; max-width: 120px; }
  .workspace-settings .auth-logout { min-height: 44px; display: inline-flex; align-items: center; }
  .site-nav .cmd-palette-trigger { width: 44px; min-width: 44px; height: 44px; padding: 0; justify-content: center; }
  .site-nav .cmd-palette-trigger-label, .site-nav .cmd-palette-trigger-kbd { display: none; }
  .site-nav .discord-btn { min-width: 44px; height: 44px; padding: 0; justify-content: center; }
  .site-nav .discord-btn span, .site-nav .donate-btn { display: none; }
  .workspace-pin-toggle, .workspace-pin-link, .workspace-unpin { min-height: 44px; }
}
@media (max-width: 760px) {
  #calendar-context-summary { display: block; margin: 4px 0 0 14px; font-size: 12px; }
  #calendar-section .card-header { margin-bottom: 8px; }
  #calendar-section .calendar-controls { margin-top: 8px; }
  .calendar-context .cal-briefing { padding: 12px; }
  .calendar-context .cal-brief-facts { grid-template-columns: 1fr; }
