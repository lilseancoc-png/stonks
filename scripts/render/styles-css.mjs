...
:root {
  /* "Ascend" — a cool near-black canvas (the trading product lives in dark
     mode). Atmosphere comes from subtle elevation steps + hairline borders,
     not gradients. The lone chromatic note is the signature spring-green
     accent; everything else is the neutral ramp below. */
  --bg:#0a0c0f;
  --surface:#171c22;
  --surface-2:#1f252d;
  --surface-3:#2a323c;
  ...
  --muted:#6a7480;
  --muted-strong:#a4aeba;
  /* Signature spring-green — the Ascend brand in one chip. Used sparingly
     for active states, focus rings, eyebrows, CTAs and the "up/positive"
     market color (unified with --pos). Red (--neg) is its only counterpart;
     never paint large fields green. */
  --accent:#16e08a;
  --accent-soft:rgba(22,224,138,0.10);
  --accent-strong:#4df0ab;
  --accent-glow:rgba(22,224,138,0.28);
  --accent-glow-soft:rgba(22,224,138,0.14);
  --accent-tint-1:rgba(22,224,138,0.06);
  --accent-tint-2:rgba(22,224,138,0.12);
  --pos:#16e08a;
  --pos-soft:rgba(22,224,138,0.10);
  --pos-glow:rgba(22,224,138,0.18);
  --pos-tint:rgba(22,224,138,0.07);
  --neg:#ff4d5e;
  --neg-soft:rgba(255,77,94,0.10);
  --neg-glow:rgba(255,77,94,0.18);
  --neg-tint:rgba(255,77,94,0.07);
  --warn:#ffb020;
  --warn-soft:rgba(255,176,32,0.12);
  --warn-tint:rgba(255,176,32,0.08);
  --info:#4c8dff;
  --info-soft:rgba(76,141,255,0.10);
  --info-tint:rgba(76,141,255,0.07);
  ...
  /* Display grotesk (Space Grotesk) — card titles, hero headlines and the
     big numbers. Slightly mechanical and confident; pairs with Hanken
     Grotesk (--font-sans) for UI and JetBrains Mono (--font-mono) for data.
     The var name stays --font-serif for compatibility with existing uses. */
  --font-serif:"Space Grotesk", "SF Pro Display", system-ui, -apple-system, sans-serif;
...
:root[data-theme="light"] {
  /* Light theme — a clean white field for marketing-style surfaces, sharing
     the same Ascend semantic tokens. The green accent darkens for legibility
     on white; P&L keeps green/red so direction stays unmistakable. */
  --bg:#ffffff;
  --surface:#ffffff;
  --surface-2:#f6f8fa;
  --surface-3:#eef1f5;
  ...
  --hairline:rgba(10,12,15,0.07);
  --text:#21262d;
  --text-strong:#0a0c0f;
  --muted:#828c99;
  --muted-strong:#515b68;
  --accent:#0e9c60;
  --accent-soft:rgba(14,156,96,0.10);
  --accent-strong:#0b7d4d;
  --accent-glow:rgba(14,156,96,0.22);
  --accent-glow-soft:rgba(14,156,96,0.10);
  --accent-tint-1:rgba(14,156,96,0.05);
  --accent-tint-2:rgba(14,156,96,0.10);
  --pos:#0e9c60;
  --pos-soft:rgba(14,156,96,0.10);
  --pos-glow:rgba(14,156,96,0.16);
  --pos-tint:rgba(14,156,96,0.06);
  --neg:#e5384b;
  --neg-soft:rgba(229,56,75,0.10);
  --neg-glow:rgba(229,56,75,0.14);
  --neg-tint:rgba(229,56,75,0.06);
  --warn:#b5800f;
  --warn-soft:rgba(181,128,15,0.10);
  --warn-tint:rgba(181,128,15,0.07);
  --info:#2563eb;
  --info-soft:rgba(37,99,235,0.10);
  --info-tint:rgba(37,99,235,0.06);
...
.narr-life-outlook-v { font-size: 11.5px; color: var(--text); line-height: 1.35; }
...
.picks-search-hint { margin: 6px 0 0; font-size: 12px; color: var(--muted); }
...
.opt-pc-key-sma50  { background: #4c8dff; }
...
.opt-pc-sma50  { stroke: #4c8dff; stroke-width: 1.2; vector-effect: non-scaling-stroke; opacity: 0.95; }
...
.opt-tech-pattern-body { font-size: 12px; color: var(--text); line-height: 1.5; }
.opt-tech-pattern-signal { font-size: 12px; color: var(--text); line-height: 1.5; }
...
.cal-ov-label { font: 600 9px/1 var(--font-mono); letter-spacing: .08em; text-transform: uppercase; color: var(--muted); }
.cal-ov-value { font: 700 16px/1.15 var(--font-serif); color: var(--text-strong); letter-spacing: -.01em; }
.cal-ov-sub { font-size: 11px; color: var(--muted-strong); overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.fomc-hl-cap { font: 600 9px/1 var(--font-mono); letter-spacing: .06em; text-transform: uppercase; color: var(--muted); }
.fomc-hl-bar {
  display: flex;
  height: 12px;
  border-radius: var(--r-pill);
...
.fomc-hl-key { display: inline-flex; align-items: center; gap: 5px; font: 600 11px/1 var(--font-mono); color: var(--muted); }
...
.fomc-pm-table { width: 100%; border-collapse: collapse; font-size: 12px; }
...
.fomc-pm-vol { font: 500 10px/1 var(--font-mono); color: var(--muted); }
...
.fomc-pm-delta { margin-left: 5px; font: 600 10px/1 var(--font-mono); }
...
.regime-hint { margin: 8px 0 12px; color: var(--muted); font-size: 12px; line-height: 1.45; }
.regime-legend {
  display: inline-flex;
  flex-wrap: wrap;
  gap: 6px 12px;
...
.regime-month-title { font: 600 12px/1 var(--font-mono); color: var(--text); margin: 0 0 6px; }
.regime-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 3px; }
.regime-dow { text-align: center; font: 600 9px/1 var(--font-mono); color: var(--muted); padding-bottom: 2px; }
.regime-cell {
  position: relative;
  aspect-ratio: 1 / 1;
  border: 1px solid transparent;
...
.regime-cell-day { font: 500 10px/1 var(--font-mono); color: var(--muted); }
...
.regime-detail-empty { color: var(--muted); font-size: 12px; }
...
.regime-detail-date { font: 600 12px/1 var(--font-mono); color: var(--text-strong); }
.regime-detail-stress { font: 500 11px/1 var(--font-mono); color: var(--muted); }
.regime-detail-summary { margin-top: 6px; font-size: 12px; line-height: 1.45; color: var(--text); }
...
.regime-detail-lean { margin-top: 8px; font-size: 12px; color: var(--text); }
...
.tape-meter-scale span { font: 700 9px/1 var(--font-mono); text-transform: uppercase; letter-spacing: .08em; color: var(--muted); }
...
.tape-meter-caption { margin-top: 7px; font: 500 11px/1.4 var(--font-sans); color: var(--muted); }
...
.acc-pc-row .acc-sym { font-size: 12.5px; }
.acc-pc-evt { font: 700 9.5px/1 var(--font-mono); letter-spacing: .03em; white-space: nowrap; }
.acc-pc-score { font: 700 11px/1 var(--font-mono); font-variant-numeric: tabular-nums; }
.acc-pc-reason { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
.acc-pc-ai { color: var(--text); font-size: 11px; overflow: hidden; text-overflow: ellipsis; }
...
.picks-position > summary {
  ...
  font: 600 12px/1.2 var(--font-sans); color: var(--muted-strong);
...
.pos-field > span { font: 600 10px/1 var(--font-sans); text-transform: uppercase; letter-spacing: .04em; color: var(--muted); }
...
.pos-check-btn {
  flex: 0 0 auto; padding: 8px 14px; font: 600 12px/1 var(--font-sans);
...
.pos-status { font-size: 12px; color: var(--muted); }
...
.pos-headline { font-size: 12.5px; color: var(--text); }
.pos-contract { font: 600 12px/1.2 var(--font-mono); color: var(--muted-strong); }
...
.pos-stat-lbl { font-size: 10px; text-transform: uppercase; letter-spacing: .03em; color: var(--muted); }
...
.pos-context-title { font: 700 10px/1 var(--font-sans); text-transform: uppercase; letter-spacing: .04em; color: var(--muted); }
...
.pos-factor-label { font: 700 10px/1 var(--font-sans); text-transform: uppercase; letter-spacing: .03em; color: var(--muted-strong); }
.pos-factor-text { font-size: 12px; line-height: 1.5; color: var(--text); }
.pos-foot { margin: 0; font-size: 11px; line-height: 1.5; color: var(--muted); }
...
@media (max-width: 640px) {
  .bonds-usd-table { font-size: 11px; }
  .bonds-usd-table th,
...
.bonds-live-change--sub { font-size: 11px; opacity: 0.85; }
...
/* Live-pace shared bits (used by the Hot stocks tab): the header status
   chip, pace heat colors, dealer-gamma line, reasoning + msg/foot text. */
.vol-live-state {
  font: 600 9px/1 var(--font-mono);
  text-transform: uppercase;
  letter-spacing: .08em;
...
/* Expanded verdict reasoning under a hot-stocks card. */
.vol-live-why {
  color: var(--text-2);
  font-size: 0.82em;
  line-height: 1.5;
...
/* Per-card dealer-gamma line — scan-time levels, live-spot distances. */
.vol-live-gex {
  color: var(--text-3);
  font-size: 0.82em;
  margin-top: 1px;
...
.vol-live-gex .is-pos { color: var(--pos); }
.vol-live-gex .is-neg { color: var(--neg); }
.vol-live-gex .vol-live-squeeze { color: var(--warn); font-weight: 600; }
.vol-live-gex-tag {
  display: inline-block;
...
/* ---- Hot stocks tab ------------------------------------------------------ */
/* The always-live top-15-by-volume-pace board. Reuses the shared live-pace
   bits above (.vol-live-state, .vol-live-pace heat colors, .vol-live-gex,
   .vol-live-why, .vol-live-msg/.vol-live-foot). */
.hot-card { padding: var(--s-4); }
.hot-board { display: grid; gap: var(--s-3); margin-top: var(--s-2); }
.hot-row {
  background: var(--surface-2);
  border: 1px solid var(--border);
  border-left: 3px solid var(--border-strong);
...
.hot-row.is-bull { border-left-color: var(--pos); }
.hot-row.is-bear { border-left-color: var(--neg); }
.hot-row.is-wait { border-left-color: var(--warn); }
.hot-row-head {
  display: flex;
  align-items: baseline;
  gap: var(--s-3);
...
.hot-sym { font-weight: 700; font-size: 1.05em; letter-spacing: 0.02em; color: var(--text); }
.hot-spot { color: var(--text-2); }
.hot-chg.is-up { color: var(--pos); }
.hot-chg.is-dn { color: var(--neg); }
/* The buy-calls / buy-puts / wait call to action. A real <button>: tapping
   it expands the verdict's reasoning inline (.vol-live-why) — hover
   tooltips don't exist on touch devices. */
.hot-verdict {
  margin-left: auto;
  padding: 2px 10px;
  background: none;
...
.hot-verdict:focus-visible { outline: none; box-shadow: var(--focus-ring); }
.hot-verdict[aria-expanded="true"] { background: var(--surface-3); }
.hot-verdict.is-bull { color: var(--pos); border-color: currentColor; background: var(--pos-soft); }
.hot-verdict.is-bear { color: var(--neg); border-color: currentColor; background: var(--neg-soft); }
...
@media (max-width: 640px) {
  .hot-card { padding: var(--s-3); }
  .hot-card > .hint { display: none; }
  .hot-row { padding: var(--s-3); }
  .hot-row-head { row-gap: var(--s-2); }
  .hot-chg { margin-left: auto; }
  .hot-verdict {
    flex-basis: 100%;
    margin-left: 0;
    order: 5;
...
.strat-leg-type button.is-on { background: var(--accent); color: #06231a; }
.strat-leg-field {
  display: flex;
  flex-direction: column;
  gap: 4px;
...
.heatmap-zoom-reset { font-size: 9px; text-transform: uppercase; letter-spacing: .08em; }
.heatmap-zoom-level {
  min-width: 40px;
  text-align: center;
  font: 600 11px/1 var(--font-mono);
...
.heatmap-legend-bar {
  ...
  height: 8px;
  border-radius: 999px;
  background: linear-gradient(to right, #ff4d5e 0%, #2a323c 50%, #16e08a 100%);
  border: 1px solid var(--border);
}
...
.brief-stat-label { font: 600 8px/1 var(--font-mono); text-transform: uppercase; letter-spacing: .06em; color: var(--muted); }
