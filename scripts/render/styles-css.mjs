...
.vol-live-play:focus-visible { outline: none; box-shadow: var(--focus-ring); }
.vol-live-play[aria-expanded="true"] { background: var(--surface-3); }
/* Expanded verdict reasoning under the row. */
.vol-live-why {
  color: var(--text-2);
  font-size: 0.82em;
  line-height: 1.5;
...
.vol-live-play.is-bull { color: var(--pos); border-color: currentColor; }
.vol-live-play.is-bear { color: var(--neg); border-color: currentColor; }
.vol-live-play.is-wait { color: var(--text-3); }
.vol-live-play.is-lean-bull { color: var(--pos); opacity: .65; }
.vol-live-play.is-lean-bear { color: var(--neg); opacity: .65; }
/* Per-row dealer-gamma line — scan-time levels, live-spot distances. */
.vol-live-gex {
  color: var(--text-3);
  font-size: 0.82em;
  margin-top: 1px;
...
/* Touch devices: a tapped row strands :hover's tint until the next tap
   elsewhere — looks like a stuck selection. (Same rationale as the global
   hover-capable-only block near the end of this file.) */
@media (hover: none) {
  .vol-live-row[data-sym]:hover { background: transparent; }
}
/* Phone layout: the 5-column grid doesn't fit a ~390px viewport — the
   volume detail drops to its own line under sym/spot/chg/pace, rows get
   44px-friendly padding + separators, and the desktop explainer hint
...
@media (max-width: 640px) {
  ...
  .vol-live-head-row { display: none; }
  .vol-live-main {
    display: flex;
    flex-wrap: wrap;
    align-items: baseline;
