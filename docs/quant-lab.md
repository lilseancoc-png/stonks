...
- **Sigma**: the `priority` ("extreme") badge needs z20 ≥ 2.5 / retZ ≥ 3 in a
  strong trend or high/crisis vol, vs 2.0/2.0 otherwise. (A literal 3σ bar on
  the Bollinger z would near-empty the screen — the 20d population-σ z is
  mathematically bounded ~4.3 — so the 3σ conditioning applies to the RETURN
  z, which genuinely reaches 3–4σ.) Priority rows sort first; inclusion bars
  (`showZ` 1.5 / `retZ` 2) are unchanged.
