...
const PREMIUM_KEYS = new Set([
  ...
  // transcripts/ prefix, gated by PREMIUM_PREFIXES below.
  "earnings-calls.json",
...
// Premium DIRECTORY prefixes — for per-ticker key families too numerous to
// enumerate. Everything under a listed prefix is premium. (iv-history/ is
// deliberately NOT here — per-ticker IV history is free like the per-ticker
// chain files.)
// - transcripts/ — the per-ticker earnings-call AI briefs
//   (data/transcripts/<SYM>.json) behind the premium Earnings calls tab.
const PREMIUM_PREFIXES = ["transcripts/"];
