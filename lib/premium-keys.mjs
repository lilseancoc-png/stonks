...
// A THIRD, stricter tier (ROLE_RESTRICTED_KEYS) gates the Track Record tab's
// data behind a SPECIFIC Discord role (api/data additionally checks the
// session's `tr` claim) — a plain premium session is not enough.
//
...
// - picks*, regime-history, briefs, trends*, unusual*, volume-flags/-history,
...
// Role-restricted = the STRICTER second tier on top of premium: the Track Record
// tab's data. A premium session is not enough — it must ALSO carry the Track
// Record role claim (api/data checks session.tr). These four are the Track
// Record tab's exclusive files (the win-rate scorecard, the in/out churn log,
// the Top-10 roster, the whole-universe grade-move log); the open-position
// marks the Top Picks "since it appeared" chip needs live in the SEPARATE,
// premium-but-NOT-role-restricted picks-open.json, so gating these does not
// touch the Top Picks tab.
//
// INVARIANT: ROLE_RESTRICTED_KEYS MUST stay a strict subset of PREMIUM_KEYS. The
...
const ROLE_RESTRICTED_KEYS = new Set([
  "picks-accuracy.json",
  "picks-changes.json",
  "picks-roster.json",
  "grades-history.json",
...
// True when the key additionally requires the Track Record role (not just any
