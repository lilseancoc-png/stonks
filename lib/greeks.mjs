...
// Yahoo expiration epochs are midnight UTC, but the contract trades until the
// ~16:00 ET close — the same 20h offset app.js applies (EXPIRY_CLOSE_OFFSET_MS),
// so expiry-day marks keep their remaining session of time value.
const EXPIRY_CLOSE_OFFSET_SEC = 20 * 3600;
export function yearsToExpiry(expirySec) {
  const nowSec = Date.now() / 1000;
  const seconds = Math.max(0, expirySec + EXPIRY_CLOSE_OFFSET_SEC - nowSec);
