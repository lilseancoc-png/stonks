...
//   * estimated 5-10 session regime-transition risk scores (0–100 weights, not calibrated probabilities);
...
function seriesReturns(data) {
  // The full bake keeps raw bars in memory; priceSeries is created only when
  // serializing ticker files. Accept both paths so live and offline builds agree.
  const bars = data?._bars?.slice(-252);
  const t = data?.priceSeries?.t || bars?.map((bar) => bar.t);
  const c = data?.priceSeries?.c || bars?.map((bar) => bar.c == null ? null : Math.round(bar.c * 100) / 100);
...
function buildCatalysts(calendar, chains, asOfDate) {
  ...
  for (const ev of calendar?.events || []) {
    if (!ev?.date) continue;
    if (ev.type === 'holiday' || /holiday/i.test(String(ev.category || '') + ' ' + String(ev.title || ''))) continue;
...
function buildTransitionLayer(regime, regimeHistory, macroHistory, macroBackdrop, currentDate = null) {
  ...
  return {
    ...
    probabilities: {
      ...
      label: "unvalidated deterministic risk score, not a calibrated probability",
    },
    fragility: {
...
function scenarioDecision(sensitivity, scenarios, transition, side = null) {
  const tradeSide = side === "put" ? "put" : side === "call" ? "call" : null;
  const direction = tradeSide === "put" ? -1 : 1;
  let weighted = 0;
  let worst = 0;
  let widest = 0;
  for (const scenario of scenarios) {
    const impact = sensitivity.scenarios[scenario.key];
    const underlyingLow = Number(impact?.low || 0);
    const underlyingMid = Number(impact?.mid || 0);
    const underlyingHigh = Number(impact?.high || 0);
    const low = direction < 0 ? -underlyingHigh : underlyingLow;
    const mid = direction < 0 ? -underlyingMid : underlyingMid;
    const high = direction < 0 ? -underlyingLow : underlyingHigh;
    weighted += mid * (scenario.probability.mid / 100);
  ...
  const exposure = tradeSide === "put" ? "this bearish exposure"
    : tradeSide === "call" ? "this bullish exposure"
      : "this exposure";
  ...
  return {
    ...
    note: fragile
      ...
      : adverse ? `Current scenario mix is an adverse filter for ${exposure}.`
        : supportive ? `Current scenario mix supports ${exposure}, subject to the ticker's own setup.`
...
export function buildScenarioEngine({
  ...
  for (const [symbol, data] of Object.entries(chains)) {
    if (!seriesReturns(data).size) continue;
  ...
  return {
    ...
    framing: "Unvalidated deterministic scenario score — a relative risk weight, not a calibrated probability or a point forecast.",
    catalysts,
    transition,
    scenarios,
    sensitivities,
    ...
    methodology: {
      probability: "Rule-based 0–100 risk scores from the current 16-axis regime, axis velocity/divergence, and upcoming events; ranges express model uncertainty, not calibrated odds.",
      ...
      limitations: [
        "Regime and scenario figures are deterministic risk scores / relative weights, not calibrated probabilities or forecasts.",
...
export function scenarioOverlayForSymbol(engine, symbol, side = null) {
  const row = engine?.sensitivities?.find((x) => x.symbol === symbol);
  if (!row) return null;
  const tradeSide = String(side || "").trim().toLowerCase();
  if (tradeSide !== "call" && tradeSide !== "put") return row.decision;
  if (!Array.isArray(engine?.scenarios) || !engine?.transition) return row.decision;
  return scenarioDecision(row, engine.scenarios, engine.transition, tradeSide);
}
