...
export async function fetchContract(symbol, expirySec, side, strike) {
  const ch = await fetchChain(symbol, new Date(expirySec * 1000));
  if (!ch) return null;
  const list = side === "put" ? ch.chain.p : ch.chain.c;
  const target = Number(strike);
  let best = null;
  let bestDiff = Infinity;
  for (const row of list) {
    if (row.s == null) continue;
    const diff = Math.abs(row.s - target);
    if (diff < bestDiff) {
      best = row;
      bestDiff = diff;
    }
  ...
  if (best && bestDiff <= 0.01) {
    return { ...best, spot: ch.spot, marketState: ch.marketState };
  }
  return null;
}
