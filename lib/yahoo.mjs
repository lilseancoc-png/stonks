...
export function withYahooTimeout(promise, label) {
  let timer;
...
// Yahoo normally keeps regularMarketPrice populated outside regular hours, so
// a null-coalescing `regular ?? post ?? pre` never surfaces the actual extended
// session price. Select by marketState first, then fall back defensively when a
// session-specific field is missing.
export function selectYahooQuotePrice(q) {
  const marketState = String(q?.marketState || "").toUpperCase();
  const regular = q?.regularMarketPrice ?? null;
  const pre = q?.preMarketPrice ?? null;
  const post = q?.postMarketPrice ?? null;
  if (marketState.startsWith("PRE") && pre != null) {
    return { spot: pre, extended: true };
  }
  if (marketState.startsWith("POST") && post != null) {
    return { spot: post, extended: true };
  }
  if (regular != null) return { spot: regular, extended: false };
  if (post != null) return { spot: post, extended: true };
  if (pre != null) return { spot: pre, extended: true };
  return { spot: null, extended: false };
}

// Option marks freeze outside the regular session. Pairing those marks with a
// pre/post-market underlying would corrupt moneyness, Greeks, and the strike
// band, so option analytics deliberately remain regular-price-first.
export function selectYahooOptionSpot(q) {
...
export async function fetchQuote(symbol) {
  ...
  // Yahoo's regularMarketChange/Percent describe the *regular session* (close
  // vs prior close). When spot is a selected pre/post-market price,
  // pairing it with the regular-session delta is inconsistent (e.g. an
  // after-hours spot shown next to a % that reflects the 4pm close). Re-derive
  // the move off prevClose so spot and the % share one baseline.
  let change = q?.regularMarketChange ?? null;
  let changePct = q?.regularMarketChangePercent ?? null;
  if (extended && spot != null) {
  ...
  // Cumulative regular-session volume so far today. The browser projects this
  // to a full-day estimate (U-shaped intraday curve) to compute a live relative
  // volume for the Execute-now card. Null outside RTH / when Yahoo omits it.
  const volume =
    q?.regularMarketVolume != null && isFinite(q.regularMarketVolume)
...
export async function fetchChain(symbol, expDate) {
  ...
  const spot = selectYahooOptionSpot(r.quote);
  ...
  // Every listed expiration (epoch sec) — lets the browser's live fallback
  // for unbaked tickers (VOO, commodity proxies) build a full expiry
  // dropdown from one call and fetch the rest per-selection.
  const expirations = (r.expirationDates || [])
    .map((d) => Math.round(new Date(d).getTime() / 1000))
