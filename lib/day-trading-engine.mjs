...
// context and already-baked technical/calendar inputs. The engine
// owns scoring, entry/exit decisions, sizing and accounting. No AI, no orders.

export const DAY_TRADING_VERSION = 2;
...
export const DAY_TRADING_RULES = Object.freeze({
  ...
  entryStartEtMin: 10 * 60,
  forceFlatEtMin: 16 * 60,
  maxHoldMinutes: 120,
  maxPositionPct: 0.25,
  baseScore: 70,
  // A neutral tape already removes 12 of the 20 market-bias points. Requiring
  // 82 on top of that made the practical bar 82/83 and made high-vol/event
  // neutral sessions mathematically impossible. 65 remains stricter than the
  // aligned 70/95 bar while allowing exceptional single-name evidence.
  neutralScore: 65,
  stock: Object.freeze({
    dailyLossPct: -0.05,
    softLossPct: -0.03,
...
const currentTimeExit = () => `${DAY_TRADING_RULES.maxHoldMinutes} minutes or the 16:00 ET close, whichever comes first`;
...
export function emptyDayTradingHistory() {
  return {
    ...
    updatedAt: null,
    portfolios: { stock: newBook("stock") },
    sessions: [],
  };
}
...
export function normalizeHistory(raw) {
  ...
  // Version 2 intentionally drops the retired 1DTE options book and its
  // session aggregates. The next scanner publication becomes stock-only in
  // both the current desk and durable tracker.
  const out = { ...base, ...source, portfolios: {} };
  out.version = DAY_TRADING_VERSION;
  for (const kind of ["stock"]) {
    out.portfolios[kind] = { ...newBook(kind), ...(source.portfolios?.[kind] || {}) };
    const book = out.portfolios[kind];
    for (const key of ["open", "closed", "resets", "equityCurve"]) {
      if (!Array.isArray(book[key])) book[key] = [];
    }
    // Open positions follow the current global time authority. Keep their
    // displayed exit plan aligned when that authority changes between scans;
    // closed rows retain the rule that was frozen when they were traded.
    for (const trade of book.open) trade.timeExit = currentTimeExit();
    for (const trade of book.closed) applyTrackRecordEligibility(trade);
  }
  out.sessions = (Array.isArray(source.sessions) ? source.sessions : []).map((session) => {
    const { optionsPnl: _optionsPnl, optionsTrades: _optionsTrades, ...stockSession } = session || {};
    return stockSession;
  });
  return out;
}

function applyTrackRecordEligibility(trade) {
  // A close on a later ET date means the scheduler missed both the time stop
  // and the mandatory 16:00 flatten. Keep the row for audit, but do not let a
  // next-session/weekend price contaminate the claimed strategy statistics.
  const openedSession = String(trade?.session || "");
  const closedMs = Date.parse(trade?.closedAt || "");
  const closedSession = Number.isFinite(closedMs) ? etClock(new Date(closedMs)).date : "";
  const crossSession = !!(openedSession && closedSession && openedSession !== closedSession);
  trade.trackRecordEligible = !crossSession;
  if (crossSession) trade.trackRecordExclusion = "missed-session-close";
  else delete trade.trackRecordExclusion;
  return trade.trackRecordEligible;
}
...
function closeQuantity(book, trade, quantity, mark, now, outcome) {
  ...
  const gross = trade.direction === "long" ? exit - trade.entryFill : trade.entryFill - exit;
  const pnl = (gross - DAY_TRADING_RULES.stock.commissionPerShare) * qty;
  trade.quantity -= qty;
...
function finalizeTrade(book, trade, mark, now, outcome) {
  ...
  applyTrackRecordEligibility(trade);
  book.closed.push(trade);
  book.closed = book.closed.slice(-5000);
  return true;
}

function markBook(book, marks, now, forceReason = null, preferFallback = false, shouldForce = null) {
  const keep = [];
  for (const trade of book.open) {
    const tradeForce = forceReason && (!shouldForce || shouldForce(trade)) ? forceReason : null;
    let mark = preferFallback && tradeForce ? forcedFallbackMark(trade) : marks.get(trade.id) || marks.get(trade.symbol);
    if (!mark && tradeForce) mark = forcedFallbackMark(trade);
    if (!mark) { keep.push(trade); continue; }
    let pnl = unrealizedPnl(trade, mark);
    if (pnl == null && tradeForce) {
      mark = forcedFallbackMark(trade);
      pnl = unrealizedPnl(trade, mark);
    }
    if (pnl == null) { keep.push(trade); continue; }
    const basis = trade.initialCash || 1;
    const ret = pnl / basis;
    const held = (now.getTime() - Date.parse(trade.openedAt)) / 60000;
    trade.lastMark = round(Number(mark.spot), 3);
...
export function scoreDayTradeCandidate(candidate, market) {
  ...
  components.push(component("Opening 30m", fhPts, 15, first.complete ? `${round(fhAvg, 2)}% average` : "opening range unavailable"));
  ...
  if (market.clock.minute < DAY_TRADING_RULES.entryStartEtMin) blocked.push("before 10:00 ET");
  if (market.clock.minute >= DAY_TRADING_RULES.forceFlatEtMin) blocked.push("session flatten in progress");
...
function bookSummary(book, marks, now) {
  const openPnl = sum(book.open, (trade) => unrealizedPnl(trade, marks.get(trade.id) || marks.get(trade.symbol)) || 0);
  const policy = bookPolicy(book);
  const allClosed = book.closed;
  const closed = allClosed.filter((trade) => trade.trackRecordEligible !== false);
  const excludedClosed = allClosed.filter((trade) => trade.trackRecordEligible === false);
  ...
  const validatedEquity = round(book.startingEquity + sum(closed, (trade) => Number(trade.pnl) || 0), 2);
  let validatedPeak = book.startingEquity;
  let validatedRunning = book.startingEquity;
  let validatedMaxDrawdown = 0;
  for (const trade of closed.slice().sort((a, b) => Date.parse(a.closedAt || 0) - Date.parse(b.closedAt || 0))) {
    validatedRunning += Number(trade.pnl) || 0;
    validatedPeak = Math.max(validatedPeak, validatedRunning);
    if (validatedPeak > 0) validatedMaxDrawdown = Math.min(validatedMaxDrawdown, (validatedRunning - validatedPeak) / validatedPeak);
  }
  ...
  return {
    ...
    rawClosedCount: allClosed.length,
    excludedClosedCount: excludedClosed.length,
    validatedEquity,
    ...
    validatedMaxDrawdownPct: round(validatedMaxDrawdown * 100, 2),
...
export function runDayTradingEngine({ history: rawHistory, candidates = [], market, marks = new Map(), now = new Date() }) {
  ...
  for (const book of Object.values(history.portfolios)) {
    // If no scheduled pass reached the prior session's close, settle stale
    // positions against their last observed mark before rolling the daily risk
    // state. This prevents an overnight quote from masquerading as intraday P&L.
    const missedSessionClose = book.open.some((trade) => trade?.session && trade.session < market.clock.date);
    if (missedSessionClose) {
      markBook(book, marks, now, "missed-session-close", true, (trade) => trade?.session && trade.session < market.clock.date);
    }
    rollBook(book, now);
  ...
  for (const row of ranked) {
    ...
    if (score.pass) {
      for (const kind of ["stock"]) {
        ...
        const trade = openStock(book, candidate, score, market, now);
  ...
  const summaries = { stock: bookSummary(history.portfolios.stock, marks, now) };
  ...
  Object.assign(session, {
    ...
    stockPnl: summaries.stock.dayRealized,
    stockTrades: history.portfolios.stock.tradesToday,
  });
  ...
  return {
    ...
    snapshot: {
      ...
      status: market.clock.minute < DAY_TRADING_RULES.entryStartEtMin ? "waiting-10:00"
        : market.clock.minute >= DAY_TRADING_RULES.forceFlatEtMin ? "entries-closed" : "scanning",
      market,
      rules: DAY_TRADING_RULES,
      portfolios: summaries,
      open: { stock: history.portfolios.stock.open },
