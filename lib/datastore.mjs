...
// dependency-free `aws4fetch`); otherwise it falls back to Vercel Blob
// (BLOB_READ_WRITE_TOKEN). This makes the R2 cutover a pure env-var flip — set
...
export function createStore(opts = {}) {
  ...
  const hasR2 = !!(r2.accountId && r2.accessKeyId && r2.secretAccessKey && r2.bucket);
  // `opts.backend` forces a specific backend (used by the Blob→R2 migration,
  // which needs BOTH live at once); otherwise auto-select R2 when its creds are
  // present, else Blob.
  const backend = opts.backend || (hasR2 ? "r2" : "blob");
  if (backend === "r2") {
    if (!hasR2) throw new Error("datastore: R2 backend requested but R2_ACCOUNT_ID/R2_ACCESS_KEY_ID/R2_SECRET_ACCESS_KEY/R2_BUCKET are incomplete");
    return createR2Store(r2);
  }
  return createBlobStore(opts);
}

// Default singleton from env. Does NOT throw at import time when nothing is
// configured — methods throw on use — so the module is safe to import anywhere.
