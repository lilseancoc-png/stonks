// Shared public/Owner data response used by both api/data/* and the authenticated
...
import { DATA_INTERNAL_PREFIX, store } from "./datastore.mjs";
import { getSession } from "./session.mjs";
import { isPremiumKey, isRetiredKey, roleClaimForKey } from "./premium-keys.mjs";
import { sanitizePublicJsonText } from "./public-data-policy.mjs";
...
export async function serveDataKey(req, res, rawKey) {
  ...
  // Removed features stay unreachable even while old immutable generations
  // and fallback-store objects age out. Stop before auth and storage access.
  if (isRetiredKey(key)) {
    return res.status(404).json({ error: "not found" });
  }

  const premium = isPremiumKey(key); // legacy name: true means Owner/internal
  ...
  try {
    ...
    if (!premium) {
      // A code deploy can precede the next data bake. Sanitize old store
      // objects here so retired Owner fields cannot survive that cutover or be
      // admitted to the public edge cache in the meantime.
      const sanitized = sanitizePublicJsonText(key, buf.toString("utf8"));
      if (sanitized != null) responseBody = sanitized;
      res.setHeader(
        "Cache-Control",
