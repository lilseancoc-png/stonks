// Open-interest gamma-exposure (GEX) proxy — server-side mirror of the browser's
...
import { expiryCloseEpochSec, greeks } from "./greeks.mjs";
...
function yearsTo(expSec, now) {
  const yrs = (expiryCloseEpochSec(expSec) * 1000 - now) / GEX_YEAR_MS;
  return Math.max(yrs, GEX_MIN_T_DAYS / 365);
}

// Total conventional-sign gamma$ across the flattened contract list at a hypothetical spot
...
// Gamma flip = the spot level where net conventional-sign GEX crosses zero. Sweep a band
...
export function computeGexSummary(chains, spot, opts = {}) {
  ...
  for (let i = 0; i < expKeys.length && exps.length < GEX_MAX_EXPS; i++) {
    const sec = expKeys[i];
    const closeMs = expiryCloseEpochSec(sec) * 1000;
