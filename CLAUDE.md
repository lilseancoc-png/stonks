...
| `AI_MODEL`, `NARRATIVES_MODEL`, `AI_FLOW_MODEL`, `AI_EOD_MODEL`, … | build / scan / refresh scripts | Override the default model per call site. **Every** AI call now defaults to `gemini-2.5-flash-lite` — the prior Gemma `AI_MODEL`/narratives base, the `gemini-2.5-flash` chart-pattern detector, and the `api/portfolio-review.js` primary/fallback were all standardized onto Flash-Lite. There are ~14 such overrides (`AI_NEWS_MODEL`, `AI_TICKER_MODEL`, `AI_CHART_MODEL`, `AI_EXIT_MODEL`, `AI_SIGNALS_MODEL`, `PORTFOLIO_REVIEW_MODEL`, `AI_RPM`, …) — `grep process.env.AI_` in `scripts/` for the full set. Prod also pins `AI_MODEL`/`NARRATIVES_MODEL` to `gemini-2.5-flash-lite` via GitHub Actions Variables (`gh variable list`), which override these code defaults. |
