...
- `buildTopPicks`, `buildGradesIndex`, `writeGradesFile`, `PICKS_MIN_CONVICTION`, `updatePicksAccuracyFile` — top-picks engine, the all-tickers grade index (`data/grades.json`), + the accuracy tracker (imported by `regen-picks.mjs`). `buildTopPicks` and `buildGradesIndex` share a `scoreAllTickers` first pass — every ticker gets the full 4-pillar score; picks keep only the actionable names, grades keep them all. `main()` builds the grade index **once** via `buildGradesIndex(...)` and reuses that single object for `writeGradesFile`, `diffGradesHistory`, and `updatePicksAccuracyFile`'s checkpoint scores — don't re-score per consumer. **The full Top Picks spec — pillars, tiers, contract selection, and the execution-timing gate — lives in [`docs/top-picks-rubric.md`](docs/top-picks-rubric.md).**
- `computeEntryTiming`, `detectMarketRegime` — the **execution-timing gate** (the `PICKS_TIMING_*` constants). `buildTopPicks` calls `computeEntryTiming(side, data, spot, { regime })` AFTER `pickContractForPick`: it reads CONFIRMED daily bars (`data._bars` in the full build, else the persisted `data.priceSeries` on regen) + technicals and returns `go`/`wait`/`avoid`. `avoid` (falling knife / chasing an extended top) DROPS the pick; `wait` ships it badged but `updatePicksAccuracyFile` only **enrolls `go` picks** (a `wait` isn't marked-to-market). It **never touches `total`** — grades/tiers/history/churn/roster are unchanged; the gate only filters the actionable roster, which may ship <10 picks (no knife backfill). In a confirmed risk-off regime it also admits reduced-size "tactical puts" (`total ≤ PICKS_RISKOFF_PUT_BAR`, gated on a `go`). Both fns are exported for unit-testing (like `resolvePickOutcome`). Server-side sibling of the browser's live `buildExecuteNowCard` (`scripts/render/app-js.mjs`) — that one reads live intraday structure, this one confirmed daily bars + the multi-day knife/chase reads the card lacks. Keep the two consistent in spirit; they are intentionally NOT a shared module (`app.js` is a generated IIFE that can't import). In a confirmed risk-off tape the gate also tightens its knife thresholds ~25%.
